...
/*
 자료형 이름(변수명)= 변수값; literal=리터럴 
 type name= literal;
 
 변수 
 	자료형
 	자료형(기본) = primitive
 	 숫자 : byte short int long (L) //기본으로 int형을 사용 
 	 문자 : char 					  // ''사용 한 글자만 들어갈 수 있음
 	 실수 : float (F), double  	 // double 기본 값 
 	 참거짓: boolean				// flase가 기본 값 
 	...
 	열거형 
 	  ...
 	  열거형 예시를 들면서 요일 , 계절 과 같이 변함 없는 예제를 사용해서 상수르 ㄹ나열하는 방법을 확인함 
 	  enum 대표이름 {
 	   		상수를 모두작성
 	   		}
...
//System.out.print와 같은 행동은 무언가를 행하기 위한 표기이기 대문에 메서드 안에 작성해줘야함 
//클래스에서 바로 작성 가능 
public class 기본변수 {
//필드
	//자료형 기본
		//1. 숫자 
			int 숫자 = 10;// int (type) 숫자 = 10;(리터럴)
			byte b = 2;
			short sh =3;
			long lng= 5L;
			
		//2. 실수 
			float ft = 3.14f; // 기본 실수 값은 double이고 double은 float 보다 큰 값이기 때문에
							  // 뒤에 f나 F를 붙여서 작성해줘야함 
			double db = 3.14; // 실수는 초기값이 double 이므로 뒤에 d를 안붙여줘도 됌 
		//3. 문자
	...
	//enum 열거형 
			//관련있는 상수형 모음집
			enum Day{
				MONDAY , THESDAY , WEDNEDAY,THURSDAY,FRIDAY,SATURDAY,SUNDAY
			}
